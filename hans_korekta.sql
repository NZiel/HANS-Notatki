...
--
-- Host: 127.0.0.1
-- Generation Time: Lis 19, 2025 at 08:49 PM
...
INSERT INTO `comments` (`id`, `note_id`, `user_id`, `content`, `created_at`) VALUES
(1, NULL, 7, 'ma ktoś pożyczyć papier milimetrowy?', '2025-11-19 14:04:11'),
(2, NULL, 1, 'Moge sprzedać ci blok za 10 zł', '2025-11-19 14:06:35'),
(3, 17, 7, 'Kradzione najlepsze', '2025-11-19 14:08:35');
...
(13, 7, 'Ruch wahadłowy i molekularny', 'Ruch wahadłowy to taki, który występuje w obiekcie z jednej strony na drugą, zwisający z włókna, kabla lub nici. Siły, które interweniują w tym ruchu, to połączenie siły grawitacji (pionowej, w kierunku środka Ziemi) i napięcia nici (kierunek nici). To właśnie robią zegary wahadłowe (stąd jego nazwa) lub huśtawki na placu zabaw. W idealnym wahadle ruch oscylacyjny będzie trwał wiecznie. Jednak w prawdziwym wahadle ruch kończy się z czasem z powodu tarcia z powietrzem. Myślenie o wahadle nieuchronnie wywołuje obraz zegara wahadłowego, pamięci tego starego i imponującego zegara wiejskiego domu dziadków. A może opowieść o terrorze Edgara Allana Poe, studni i wahadle, której narrację inspiruje jedna z wielu metod tortur stosowanych przez hiszpańską inkwizycję.', NULL, '2025-11-09 13:28:15', '2025-11-19 14:18:22'),
...
CREATE TABLE `ratings` (
  ...
  `note_id` int(11) NOT NULL,
  `user_id` int(11) NOT NULL,
  `rating` tinyint(1) NOT NULL COMMENT 'Ocena: 1 do 5',
  `created_at` datetime DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
ALTER TABLE `ratings`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `note_user_unique` (`note_id`,`user_id`),
  ADD KEY `user_id` (`user_id`);
